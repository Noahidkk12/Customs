...
local ConfigFileName = "VaultsHub_DUELS_Config.json"
local Enabled = {
    SpeedBoost = false,
    AntiRagdoll = false,
    SpinBot = false,
...
coordESPFolder.Name = "VaultsHub_CoordESP"
...
-- Create markers at exact coordinates (CHANGED TO MONOCHROME)
createCoordMarker(Vector3.new(-476.48, -6.28, 92.73), "L1", Color3.fromRGB(200, 200, 200))
createCoordMarker(Vector3.new(-483.12, -4.95, 94.80), "L END", Color3.fromRGB(255, 255, 255))
createCoordMarker(Vector3.new(-476.16, -6.52, 25.62), "R1", Color3.fromRGB(180, 180, 180))
createCoordMarker(Vector3.new(-483.04, -5.09, 23.14), "R END", Color3.fromRGB(220, 220, 220))
...
-- Ragdoll TP UI (drop this in your hub script, call the functions from your button)
...
local function holoOutline(obj)
    local stroke = Instance.new("UIStroke")
    stroke.Thickness = 2.2
    stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    stroke.Color = Color3.fromRGB(190, 100, 255)
    stroke.Parent = obj
    ...
    task.spawn(function()
        ...
        while obj.Parent do
            ...
            grad.Offset = Vector2.new(math.sin(t * 1.3) * 0.7, 0)
        end
    end)
end

local function holoText(label)
    local grad = Instance.new("UIGradient")
    grad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,   Color3.fromRGB(180,  90, 255)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 120, 200)),
    ...
    task.spawn(function()
        ...
        while label.Parent do
            t += RunService.RenderStepped:Wait()
            grad.Offset = Vector2.new(math.sin(t * 1.25) * 0.65, 0)
        end
    end)
end
...
screenGui.Name = "VaultsHubTp_UI"
screenGui.ResetOnSpawn = false
screenGui.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui")  -- only place we touch player
...
holoOutline(mainFrame)

local title = Instance.new("TextLabel")
title.Size               = UDim2.new(1, 0, 0, 17)
title.Position           = UDim2.new(0, 0, 0, 10)
title.BackgroundTransparency = 1
title.Text               = "VaultsHub ragdoll tp"
...
local function createBtn(text, pos)
    ...
    local normalColor  = Color3.fromRGB(22, 22, 28)
    local normalTrans  = 0.25
    local pressColor   = Color3.fromRGB(165, 100, 255)
    local pressTrans   = 0.05
    ...
    btn.MouseLeave:Connect(function()
        if btn.BackgroundColor3 ~= pressColor then
            btn.BackgroundTransparency = normalTrans
        end
    end)
    ...
    btn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            btn.BackgroundTransparency = pressTrans
        end
    end)
...
local tpButton1 = createBtn("TP Left",  UDim2.new(0.05, 0, 0.45, 0))
local tpButton2 = createBtn("TP Right", UDim2.new(0.53, 0, 0.45, 0))
...
-- ============================================
-- GUI - CLEAN NO BOXES - MONOCHROME / BLACK & WHITE
-- ============================================
local isMobile = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
local guiScale = isMobile and 0.4 or 1

-- REDEFINED COLORS FOR MONOCHROME THEME
local C = {
    bg = Color3.fromRGB(10, 10, 10), -- Dark Gray/Black
    purple = Color3.fromRGB(255, 255, 255), -- White (Primary Accent)
    purpleLight = Color3.fromRGB(200, 200, 200), -- Light Gray
    purpleDark = Color3.fromRGB(50, 50, 50), -- Dark Gray
    purpleGlow = Color3.fromRGB(150, 150, 150), -- Gray
    accent = Color3.fromRGB(255, 255, 255), -- White
    text = Color3.fromRGB(255, 255, 255), -- White
    textDim = Color3.fromRGB(150, 150, 150), -- Gray
    success = Color3.fromRGB(200, 200, 200), -- Light Gray
    danger = Color3.fromRGB(100, 100, 100), -- Gray
    border = Color3.fromRGB(60, 60, 60) -- Gray border
}

local sg = Instance.new("ScreenGui")
sg.Name = "VaultsHub_BW"
...
titleLabel.Text = "VaultsHub Duels"
...
subtitleLabel.Text = "https://discord.gg/pQgNdUatcr"
...
local function createToggleWithKey(parent, yPos, labelText, keybindKey, enabledKey, callback, specialColor)
    ...
    keyBtn.TextColor3 = Color3.fromRGB(235, 235, 235) -- Black text on white button
    ...
    toggleBg.BackgroundColor3 = defaultOn and onColor or Color3.fromRGB(35, 35, 35)
    toggleBg.ZIndex = 4
...
local function setVisual(state, skipCallback)
    ...
    if state then
        -- ВКЛЮЧЕНО: белый фон + чёрный кружок
        TweenService:Create(toggleBg, TweenInfo.new(0.25), {
            BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        }):Play()
        
        TweenService:Create(toggleCircle, TweenInfo.new(0.28), {
            BackgroundColor3 = Color3.fromRGB(0, 0, 0),
            Position = UDim2.new(1, -23 * guiScale, 0.5, -10 * guiScale)
        }):Play()
    else
        -- ВЫКЛЮЧЕНО: серый фон + белый кружок
        TweenService:Create(toggleBg, TweenInfo.new(0.25), {
...
-- Left side toggles - MONOCHROME ARGS
...
createToggleWithKey(leftSide, 165, "Spin Bot", "SPIN", "SpinBot", function(s)
    Enabled.SpinBot = s
    if s then startSpinBot() else stopSpinBot() end
end)

createSlider(leftSide, 216, "Spin Speed", 5, 50, "SpinSpeed", function(v) Values.SpinSpeed = v end)

createToggle(leftSide, 275, "Spam Bat", "SpamBat", function(s)
    Enabled.SpamBat = s
...
createToggleWithKey(leftSide, 380, "Bat Aimbot", "BATAIMBOT", "BatAimbot", function(s)
    Enabled.BatAimbot = s
...
end, Color3.fromRGB(255, 255, 255)) -- Pure White instead of Blue
...
end, Color3.fromRGB(180, 180, 180)) -- Light Gray
...
end, Color3.fromRGB(150, 150, 150)) -- Medium Gray
...
SaveBtn.TextColor3 = Color3.new(0, 0, 0) -- Black text on white
